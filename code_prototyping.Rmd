...
datfinal <- left_join(wysf, dat, by = "EvaluationID")
...
```{r}
mapVar <- "PctBankCoveredMIMCondition"
...
pal <- colorFactor(c("green", "yellow", "red"), domain = c("Minimal", "Moderate", "Major"))
    ...
    datfinal %>%
      ...
      addLayersControl(
        ...
        # position it on the topleft
        position = "topleft") %>%
      addLegend(pal = pal, values = ~datfinal$PctBankCoveredMIMCondition, opacity = 1, title = "test")
